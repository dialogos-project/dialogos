...
public class Main implements MenuCommander, MenuOwner, Commands {
  ...
  private static Collection<DocumentWindow<?>> documents =
    new ArrayList<DocumentWindow<?>>();

  private final FileChooser fileChooser = new FileChooser(Preferences
      .getPrefs().getLastUsedFile());

  private RequiredEventHandler systemEventHandler = new RequiredEventHandler(
      true, true) {
   ...
   * 
   * @param d
   *          DocumentWindow to be added to the list of listeners
   ...
   * 
   * @param d
   *          reference on the document window.
   * @param f
   *          File to be opened.
   ...
   * 
   * @param d
   *          Document for which a window should be created.
   * @exception IllegalArgumentException
   *              if the type of the document was neither MultiDocument, nor
   *              LogDocument, nor SingleDocument.
