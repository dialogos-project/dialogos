...
public class GUI {
    ...
    private static DynamicResourceBundle resources = new DynamicResourceBundle( "com.clt.gui.Resources", new Runnable() {
        public void run() {
    ...
    public static void scrollIntoCenter(Rectangle r, JViewport view) {
        ...
        Point optimalPosition = new Point(r.x + (r.width - d.width) / 2, r.y + (r.height - d.height) / 2);
        // System.out.print(optimalPosition + " -> ");
        optimalPosition.x = Math.max(0, Math.min(s.width - d.width, optimalPosition.x));
        optimalPosition.y = Math.max(0, Math.min(s.height - d.height, optimalPosition.y));
        /*
    ...
    public static JScrollPane createScrollPane(JComponent view, int vScrollBarPolicy, int hScrollBarPolicy, final int maxWidth, final int maxHeight) {
        ...
        JScrollPane jsp = new JScrollPane(view, vScrollBarPolicy, hScrollBarPolicy) {
            @Override
    ...
    public static JScrollPane createScrollPane(JComponent view, int vScrollBarPolicy, int hScrollBarPolicy) {
        return GUI.createScrollPane(view, vScrollBarPolicy, hScrollBarPolicy, Integer.MAX_VALUE, Integer.MAX_VALUE);
    }
    ...
    public static JScrollPane createScrollPane(final JTable table, final Dimension size) {
        final JScrollPane jsp = new JScrollPane(table,
            ...
            @Override
            public Dimension getPreferredSize() {
                return new Dimension(size.width > 0 ? size.width : super.getPreferredSize().width,
                        size.height > 0 ? size.height : super.getPreferredSize().height);
            }
        };
        ...
        if (scroller != null) {
            ...
            scroller.addAdjustmentListener(new AdjustmentListener() {
                ...
                public void adjustmentValueChanged(AdjustmentEvent e) {
                    ...
                    boolean enabled = ((diff > 0) && (scroller.getVisibleAmount() < diff));
    ...
    public static boolean isPopupTrigger(MouseEvent e) {
        ...
        return e.isControlDown() || ((modifiers & InputEvent.BUTTON3_DOWN_MASK) != 0);
    }
    ...
    public static void setWaitCursor(Component c) {
        ...
        if (c == null) {
            throw new IllegalArgumentException("Can't change cursor on <null> component");
        }
        Cursor cursor = Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR);
        if (cursor == null) {
            throw new MissingResourceException("Failed to load wait cursor", Cursor.class.getName(), "WAIT_CURSOR");
        }
        c.setCursor(cursor);
    }
     ...
     * exceptions. Use
     * <code>invokeAndWaitCanThrow(Runnable r) throws Throwable</code> if you
     * want to catch exceptions.
     *
     * @param r Runnable to be executed by the event dispatching thread.
     */
