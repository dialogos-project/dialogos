...
public class AssignmentDialog<SRC_TYPE, DST_TYPE>
    extends JDialog
  ...
  private AssignmentDialog(Component parent, Collection<SRC_TYPE> src,
      Collection<DST_TYPE> dst) {
    ...
    JButton b1 =
      new CmdButton(this, Commands.cmdCancel, GUI.getString("Cancel"));
    ...
    Dimension d =
      new Dimension(Math.max(d1.width, d2.width), Math
  ...
  public static <X, Y> Map<X, Y> getAssignment(Component parent,
      Collection<X> src,
            Collection<Y> dst) {
  ...
  private class AssignmentModel
    ...
    public boolean moveRow(int draggedColumn, int sourceRow, int targetRow) {
      ...
      if (draggedColumn == 0) {
        ...
        AssignmentDialog.this.src.set(sourceRow, AssignmentDialog.this.src
          .get(targetRow));
        AssignmentDialog.this.src.set(targetRow, tmp);
      }
      else if (draggedColumn == 2) {
        DST_TYPE tmp = AssignmentDialog.this.dst.get(sourceRow);
        AssignmentDialog.this.dst.set(sourceRow, AssignmentDialog.this.dst
          .get(targetRow));
