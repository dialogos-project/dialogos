...
public class TextStreamReader
    ...
    synchronized (this.lock) {
      if (this.codepage == Codepages.UTF8) {
        ...
        for (int i = 0; i < result; i++) {
          cbuf[off + i] =
            Codepages.ToUnicode[this.codepage][this.byteBuffer[i] & 0x000000ff];
        }
  ...
  private static class UTFReader {
    ...
    private boolean checkBytes(int bytesNeeded)
        throws IOException {
    ...
    public int read(char cbuf[], int off, int len)
      ...
      for (j = 0; j < len; j++) {
        ...
        if (c >= 0xE0) {
          ...
          cbuf[j + off] =
            (char)(((this.bytes[this.index] & 0x0F) << 12)
          ...
          cbuf[j + off] =
            (char)(((this.bytes[this.index] & 0x1F) << 6) | (this.bytes[this.index + 1] & 0x3F));
          this.index += 2;
        }
        else {
          cbuf[j + off] = (char)c;
