...
public class Base64 {
  ...
  public static byte[] decode(byte[] source, int off, int len) {
    ...
    for (i = off; i < off + len; i++) {
      ...
      if (sbiDecode >= Base64.WHITE_SPACE_ENC) // White space, Equals sign or
      // better
      {
  ...
  public static byte[] decode(String s) {
    ...
    if ((bytes != null) && (bytes.length >= 4)) {
      ...
      int head = (bytes[0] & 0x000000ff) | ((bytes[1] << 8) & 0xff00);
  ...
  public static class InputStream
    ...
    public InputStream(java.io.InputStream in, int options) {
      ...
      super(in);
      this.breakLines =
        (options & Base64.DONT_BREAK_LINES) != Base64.DONT_BREAK_LINES;
  ...
  public static class OutputStream
    ...
    public void flushBase64()
      ...
      if (this.position > 0) {
        if (this.encode) {
          this.out
            .write(Base64.encode3to4(this.b4, this.buffer, this.position));
          this.position = 0;
        } // end if: encoding
        else {
