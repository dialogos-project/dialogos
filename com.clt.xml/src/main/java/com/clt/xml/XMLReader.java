...
public class XMLReader {
  ...
  private Collection<XMLProgressListener> progressListeners =
    new ArrayList<XMLProgressListener>();
   ...
   * 
   * @param validating
   *          Whether the parser should be validating.
   */
  ...
  /**
   * Add a progress listener that should be notified of progress during a parse.
   * 
   * @param l
   *          an XMLProgressListener that should be notified of progress during
   *          a parse.
   */
   ...
   * 
   * @param l
   *          Reference on the XMLProgressListener to be removed.
   */
   ...
   * handler's name may be <code>null</code>. In this case the document will be
   * parsed, no matter what the main element is. If the name is not
  ...
  /**
   * Parse the given file using the given top level content and error handlers.
   * The handler's name may be <code>null</code>. In this case the document will
   ...
   * document will be parsed, no matter what the main element is. If the name is
   * not <code>null</code> it must match the main element of the document.
   * 
   * @throws IOException
   */
  ...
  /**
   * Install a subordinate content handler for the current element. This method
   * should be called from within the {@link XMLHandler#startElement} callback
   * during a parse to handle that element. As soon as the end tag for that
   * element is reached, the handler is discarded and control is returned to the
   * previously active handler. Several handlers can be stacked in such a way.
   * Keep in mind though, that the parser will simply look for the name of an
   * end tag in order to decide whether it should discard the current handler.
   * This is a problem when you try to parse elements that can recursively
   * contain elements of the same name. You can circumvent this problem be
   * recursively installing subordinate handlers whenever you encounter the
   * start of an element with the same name.
   * 
   * @param handler
   */
   ...
   * end of the given element is reached. This method handles recursive elements
   * correctly.
   * 
   * @param element
   */
