...
public class Greater extends BinaryOperator {
    ...
    public Expression copy(Map<?, ?> mapping) {
        ...
        return new Greater(this.e1.copy(mapping), this.e2.copy(mapping));
    }
    ...
    protected Value eval(Debugger dbg) {
        ...
        if ((v1 instanceof IntValue) && (v2 instanceof IntValue)) {
            ...
            return (new BoolValue(((RealValue) v1).getReal() > ((IntValue) v2).getInt()));
        } else if ((v1 instanceof IntValue) && (v2 instanceof RealValue)) {
            return (new BoolValue(((IntValue) v1).getInt() > ((RealValue) v2).getReal()));
