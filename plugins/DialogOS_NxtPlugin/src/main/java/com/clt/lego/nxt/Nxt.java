...
public interface Nxt extends Brick {
     ...
     * If <code>wait</code> is true, this method will only return when the tone
    ...
    /**
     * Stop any running program on the brick. Return true if a program was
     * stopped or false if no program was runnning.
     *
     */
    public boolean stopProgram() throws IOException;

    /**
     * Return the name of the currently running program or <code>null</code> if
    ...
    public void setOutputState(Motor.Port motor, int power, int mode, Motor.Regulation regulation, int turnRatio, Motor.State state, long tachoLimit)
            throws IOException;
