...
public class SerialPort {
  ...
  public void openForNxt()
      throws IOException {

    this.open(SerialPort.BAUDRATE_BLUETOOTH, gnu.io.SerialPort.DATABITS_8,
      gnu.io.SerialPort.STOPBITS_1, gnu.io.SerialPort.PARITY_NONE);
  }


  public void openForRcx()
      throws IOException {

    this.open(SerialPort.BAUDRATE_RCX, gnu.io.SerialPort.DATABITS_8,
      gnu.io.SerialPort.STOPBITS_1, gnu.io.SerialPort.PARITY_NONE);
  }


  public void open(int baudRate, int dataBits, int stopBits, int parity)
      throws IOException {
    ...
    if (this.portIdentifier.isCurrentlyOwned()) {
      ...
      try {
        port =
          this.portIdentifier.open(this.portIdentifier.getName(),
            SerialPort.CONNECTION_TIMEOUT);
      } catch (PortInUseException exn) {
        throw new IOException("Port is already in use by "
                        + this.portIdentifier.getCurrentOwner());
      }
      if (port instanceof gnu.io.SerialPort) {
        gnu.io.SerialPort serialPort = (gnu.io.SerialPort)port;
        try {
        ...
        port.close();
        throw new IOException(this.portIdentifier.getName()
