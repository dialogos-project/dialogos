...
class Rendezvous {
    ...
    try {
      ...
      Rendezvous.addServiceListener(new ServiceListener() {
        ...
        public void removeService(JmDNS jmdns, String type, String name) {
          ...
          if (type.equals(Rendezvous.SERVICE_TYPE)) {
            name = this.normalizeName(name, type);
            ...
            ServiceInfo info)
                {
                  ...
                  if (type.equals(Rendezvous.SERVICE_TYPE) && (info != null)) {
                    name = this.normalizeName(name, type);
                    synchronized (Rendezvous.clientList) {
                      Rendezvous.clients.put(name, info);
                      Rendezvous.clientList.update();
                    }
                  }
                }
        ...
        /*
         * // new jmdns API public void serviceAdded(ServiceEvent evt) { if
         * (evt.getInfo() == null) addService(evt.getDNS(), evt.getType(),
         * evt.getName()); }
         * 
         * public void serviceRemoved(ServiceEvent evt) {
         * removeService(evt.getDNS(), evt.getType(), evt.getName()); }
         * 
         * public void serviceResolved(ServiceEvent evt) {
         * resolveService(evt.getDNS(), evt.getType(), evt.getName(),
         * evt.getInfo()); }
         */
      });
    } catch (Exception exn) {
  ...
  private static void initRendezvous()
      throws IOException {
    ...
    try {
      ...
      if (Rendezvous.VERBOSE) {
        System.err
          .println("Could not determine network interfaces. Using localhost.");
      }
    ...
    for (int i = 0; i < addr.length; i++) {
      Rendezvous.rendezvous[i] = new JmDNS(addr[i]);
      Rendezvous.rendezvous[i].registerServiceType(Rendezvous.SERVICE_TYPE);
      if (Rendezvous.VERBOSE) {
        System.out.println("Rendezvous activated on interface "
                      + Rendezvous.rendezvous[i].getInterface());
      }
    }
  }
  ...
  public static ServiceInfo createService(String name, int port,
    ...
    return new ServiceInfo(Rendezvous.SERVICE_TYPE, name + "."
      + Rendezvous.SERVICE_TYPE, port, 0, 0, properties);
  }
  ...
  static ServiceInfo[] getServiceInfo(String name, String server)
      throws IOException {
    ...
    for (ServiceInfo info : Rendezvous.clients.values()) {
      String clientName =
        info.getPropertyString(Rendezvous.SERVICE_PROPERTY_NAME);
      boolean local =
        "yes".equals(info.getPropertyString(Rendezvous.SERVICE_PROPERTY_LOCAL));
      /*
      ...
      if ((name == null) || info.getName().equals(name)
                    || ((clientName != null) && clientName.equals(name))) {
        if (local ? info.getAddress().equals(
