...
/**
 * The base class for clients to the CLT dialog tool.
 *
 * The Client Interface provides:
 * <ul>
 * <li>Communications protocol</li>
 * <li>Synchronization</li>
 * <li>Server name resolution</li>
 * </ul>
 * 
 * Clients receive data from the dialog server and present this data to the
 * user. This may include visual feedback, text-to-speech, sound playback etc.
 *
 * Implementing a client requires three steps:
 * <ol>
 * <li>Create a new class that inherits from <code>Client</code>
 * <li>Override a couple of callbacks to implement client behaviour
 * <li>Call <code>open</code> on the client class to activate it.
 * </ol>
 *
 *
 * Clients must implement the following callbacks:
 * <ul>
 * <li>{@link #getName}</li>
 * <li>{@link #stateChanged}</li>
 * <li>{@link #sessionStarted}</li>
 * <li>{@link #reset}</li>
 * <li>{@link #error}</li>
 * <li>{@link #waitUntilDone}</li>
 * <li>{@link #output}</li>
 * </ul>
 *
 * Optionally you may override {@link #rpc} to provide remote procedures.
 *
 * <p>
 * By implementing {@link #getName}, you provide a name for your client. This
 * name will be used by DialogOS to find your client, i.e. you must enter this
 * name in the <code>Service Name</code> field in DialogOS.
 * </p>
 *
 * <p>
 * <code>output</code> messages from the server are internally queued and your
 * callbacks are handled synchronously. This means that you will only get the
 * next output message after you return from the callback. If you want to
 * trigger a long lasting action and process further output messages at the same
 * time, you have to start a new thread from your handler.
 * </p>
 *
 * <blockquote>
 * <strong>NOTE:</strong> This is only important for your <code>output</code>
 * handler. The <code>reset()</code>, <code>sessionStarted()</code>,
 ...
 * data. </blockquote>
 *
 * <p>Example: A sample player that receives names of files to
 * play via <code>output</code> should start playing the file and only return
 * when the sample has finished playing or your <code>reset</code> callback is
 * called (in this case you should abort the currently playing sample).
 * </p>
 *
 * <p>
 ...
 * these threads have finished execution.
 * </p>
 *
 * <p>
 * Clients can send data back to the server. This data may include results of a
 * speech recognizer, sensor input etc. Simply call <code>send</code> to send
 * data to the server. The server will internally queue and filter your data, so
 * you can call <code>send</code> at any time.
 * </p>
 *
 * <p>
 * Here's an example of a client that simply echoes everything it receives:
 * <p>
 *
 * <pre>
...
public abstract class Client {
  ...
  /**
   * Notification that the client has changed its state.
   * 
   * @param state
   ...
   * setup its internal state for the beginning of a new dialog session.
   * 
   * This callback should not be used for initialisation actions that takes more
   * than a second because the server expects the client to be active
   * immediately.
   */
   ...
   * should stop any pending output. This message is sent when the "Send reset"
   * option is enabled for an OutputNode. The intent is to clear any pending
   * output before producing a new output.
   */
   ...
   * of {@link com.clt.script.exp.Value}. If you start a new thread from
   * <code>output</code> you need to override <code>waitUntilDone</code>.
   * 
   * @param value
   *          the data to output
   * 
  ...
  /**
   * Request that the client should executed te given procedure. Clients may
   * override this method to provide their own procedures. If you can't resolve
   * a procedure, because the name is unknown or the number or type of arguments
   * do not match, you should call <code>super.rpc(procedure, arguments)</code>.
   * If you throw an exception while handling the call, you should provide a
   * descriptive message of the cause of the error with your exception.
   * 
   * @param procedure
   *          the name of the procedure to call
   * @param arguments
   *          the (possibly empty) array of arguments
   * 
   * @return The result of executing the given procedure with the given
   *         arguments. You may return <code>null</code> or
   *         {@link com.clt.script.exp.values.Undefined}, if your procedure has
   ...
   * notification with an argument value of <code>true</code> and only until you
   * receive this message with a value of <code>true</code>.
   */
  ...
  /**
   * Send a timeout signal to the dialog manager. You should do so only, if you
   * received a message, that timeouts are allowed.
   * 
   * @see #allowTimeout
   * @throws IOException
   */
  ...
  /**
   * Return a decriptive name for this client.
   */
   ...
   * output. The default implementation assumes that you haven't started any new
   * threads from your <code>output</code> handler so it simply returns. If you
   * start your own threads from <code>output</code> you need to override
   * <code>waitUntilDone</code> and make sure that your threads have finished
   * before <code>waitUntilDone</code> returns.
   */
   ...
   * The map's keys are converted to Strings using <code>toString()</code></td>
   * </tr>
   * </table>
   ...
   * applied recursively, so a <code>java.util.Map[]</code> will be converted to
   * a {@link com.clt.script.exp.values.ListValue} containing
   * {@link com.clt.script.exp.values.StructValue}s whose labels are the keys of
   * the respective Map (converted to String) and whose values are the converted
   * values of the Map.
   * 
   * @param data
   *          the data to send
   * @exception IOException
   *              if writing to the socket failed
   */
   ...
   * 
   * @exception IOException
   *              If starting up the network interface failed.
   * 
   * @see #close
   */
  public void open()
  ...
  /**
   * Start the client. This method is only provided for compatibility purposes.
   * Please use {@link #open()} or {@link #open(int)} instead.
   * 
  ...
  /**
   * Stop the client. This will abort any active connection and close the client
   * port. No further connection attempts will be made. You can reopen the
   * client at any time using <code>open</code>.
   * 
   * @see #open()
   * @see #open(int)
   */
   ...
   * Rendezvous. The default returns true. Override to make your client private
   * (i.e. only visible by servers running on the same machine). The client will
   * still be accessible for servers that connect directly to the client's port.
   * 
   * @see #useRendezvous()
   */
