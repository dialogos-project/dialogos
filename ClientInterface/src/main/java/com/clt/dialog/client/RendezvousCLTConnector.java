/*
 * @(#)RendezvousCLTConnector.java
 * Created on Mon Jun 28 2004
 *
...
public class RendezvousCLTConnector
    extends CLTConnector {

  private StringProperty SERVICE_NAME =
    new DefaultStringProperty("Service Name", "Service Name", null);
  private StringProperty SERVER_NAME =
    new DefaultStringProperty("Server", "Server", null);
  ...
  public RendezvousCLTConnector copy() {
    ...
    return new RendezvousCLTConnector(this.SERVICE_NAME.getValue(),
      this.SERVER_NAME.getValue());
  }
  ...
  protected Connection createConnection(TargetSelector selector)
    ...
    ServiceInfo[] services =
      Rendezvous.getServiceInfo(this.SERVICE_NAME.getValue().trim(),
            this.SERVER_NAME.getValue().trim());
    if (services.length == 0) {
      throw new ConnectException("Service " + this.SERVICE_NAME.getValue()
        + " not found");
    }
    else {
      ...
      if (services.length == 1) {
        ...
        ServiceInfoDescription[] sid =
          new ServiceInfoDescription[services.length];
        ServiceInfoDescription preferredService = null;

        List<String> localAddresses =
          Arrays.asList(Rendezvous.getLocalHostAddresses());
      ...
      String protocols =
        service.getPropertyString(Rendezvous.SERVICE_PROPERTY_PROTOCOL);
      ...
      final ServiceInfoDescription selectedService =
        new ServiceInfoDescription(service, false);
      return new SocketConnection(new Socket(service.getAddress(), service
        .getPort()), protocol) {
