...
public abstract class AbstractRecognizer implements Recognizer, G2P {
  ...
  /**
   * Starts the speech recognizer. This method should do whatever is needed
   * to start up the underlying speech recognition engine. It should then read
   * audio input from the microphone and feed it to the speech recognizer.
   * Once speech is finished, it should return a {@link RecognitionResult} containing
   * the recognized string. Note that one RecognitionResult may contain a number
   * of different hypotheses, each with its own score. Note also that DialogOS
   * can be configured to reject recognition results below a certain confidence
   * threshold. The confidences of each alternative in the recognition result
   * will be measured against this.<p>
   * 
   * An implementation of startImpl should call {@link #fireRecognizerEvent(com.clt.speech.recognition.RecognizerEvent) }
   * to indicate the status of the speech recognizer. The status will be displayed
   * in the DialogOS GUI (green microphone, etc.). In particular, events
   * should be fired when the recognizer becomes READY, ACTIVATED, or DEACTIVATED
   * (see {@link RecognizerEvent} for details).<p>
   * 
   * When the recognizer is cancelled from the GUI, {@link #stopImpl() } will be
   * called to terminate the recognizer and initiate whatever cleanup is necessary.
   * Some of this cleanup may take place in startImpl. In such a case, startImpl
   * is expected to return null, in order to indicate to the recognition thread
   * that the recognizer GUI can be closed and the dialog finished.
   * 
   * @return
   * @throws SpeechException 
   */
  ...
  /**
   * Stops the speech recognizer. This method should do whatever is needed
   * to terminate the recognizer and perform any necessary cleanup.
   * 
   * @throws SpeechException 
   */
  ...
  /**
   * @param contexts
   *          A collection of {@link RecognitionContext}s
   */
  protected abstract void saveContextCache(
      Collection<RecognitionContext> contexts,
            File grammarDirectory)
      throws SpeechException, IOException;


  protected abstract RecognitionContext createContext(String name, Grammar g,
      Domain domain,
            long timestamp)
      throws SpeechException;


  public abstract RecognitionContext createTemporaryContext(Grammar g,
      Domain domain)
        throws SpeechException;


  public final void setContext(Grammar grammar)
      throws SpeechException {
